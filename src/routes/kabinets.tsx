...
function KabinetsPage() {
  ...
  const bookingsQ = useQuery({
    ...
    queryFn: async () => {
      ...
      if (!res.ok) {
        throw new Error(res.error === "db_unavailable" ? "db_unavailable" : "list_failed");
      }
      ...
      <div className="mx-auto max-w-lg space-y-10 px-4 py-10">
        ...
        <section>
          ...
          {bookingsQ.isLoading ? (
            ...
            <div className="rounded-xl border border-danger/30 bg-danger/5 px-5 py-6 text-center">
              <p className="text-sm text-danger">{t("dbUnavailable")}</p>
              <Button className="mt-4" variant="outline" size="md" onClick={() => void bookingsQ.refetch()}>
                {t("retry")}
              </Button>
            </div>
          ) : bookings.length === 0 ? (
