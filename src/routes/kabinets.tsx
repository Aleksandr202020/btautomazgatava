...
function vehicleLabel(v: string, lang: string): string {
  const map: Record<string, Record<string, string>> = {
    car: { lv: "Vieglais auto", ru: "Легковой", en: "Car" },
    large_car: { lv: "Jeep / Crossover", ru: "Джип / Кроссовер", en: "Large car" },
    commercial: { lv: "Komerc", ru: "Коммерческий", en: "Commercial" },
    suv: { lv: "Jeep / Crossover", ru: "Джип / Кроссовер", en: "SUV" },
  };
  return map[v]?.[lang] ?? v;
}
...
function BookingCard({ b, lang, t, past, onEdit, onCancel, cancelling }: { b: BookingPublic; lang: string; t: (k: any) => string; past?: boolean; onEdit?: () => void; onCancel?: () => void; cancelling?: boolean }) {
      ...
      <div className="relative z-10 max-h-[90dvh] w-full max-w-md overflow-y-auto rounded-t-2xl bg-white p-5 shadow-xl sm:rounded-2xl">
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <div className="grid grid-cols-1 gap-2">
              {(["car", "large_car", "commercial"] as const).map((v) => (
                <button key={v} type="button" onClick={() => setVehicleType(v)} className={cn("rounded-lg border px-3 py-2.5 text-left text-sm font-medium transition", vehicleType === v || (v === "large_car" && vehicleType === "suv") ? "border-accent bg-accent/10 text-accent" : "border-border text-fg hover:bg-elevated")}>
                  {vehicleLabel(v, lang)}
                </button>
              ))}
            </div>
          ...
          <div className="flex gap-2 pt-2">
            <Button className="flex-1" size="lg" disabled={!date || !time || saveMut.isPending} onClick={() => saveMut.mutate()}>
              {t("saveChanges")}
            </Button>
