    ...
    <div className="flex w-full max-w-sm flex-col gap-2">
      {GROK_PROVIDERS.map((p) => (
        <button
          ...
          className="inline-flex h-12 w-full cursor-pointer items-center justify-center rounded-md border border-border bg-elevated px-4 text-sm font-medium tracking-[0.06em] text-fg transition hover:border-fg"
        >
          {p.label === "Google" ? "Google" : p.label === "X" ? "X" : p.label}
        </button>
      ))}
    </div>
  );
