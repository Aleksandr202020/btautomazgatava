...
/** Own bookings for the signed-in user (cabinet). */
...
export const updateMyBooking = createServerFn({ method: "POST" })
  .middleware([authMiddleware])
  .validator((input: unknown) => UpdateMyZ.parse(input))
  .handler(async ({ context, data }) => {
    try {
      const { getSql } = await import("@/lib/db");
      const { getSessionUser } = await import("@/lib/auth/verify.server");
      ...
      if (!isWorkingDate(nextDate) || isSlotInPast(nextDate, nextTime)) {
        return { ok: false as const, error: "slot_taken" as const };
      }
      const validTimes = generateSlots();
      ...
      if (data.vehicleType || data.carBrand || data.carModel || data.carPriceCategory) {
        const resolved = getVehiclePrice(
          ...
          data.carModel || "",
          (data.carPriceCategory as PriceCategory | undefined) ??
            (normalizeVehicleId(data.vehicleType ?? row.vehicle_type) as PriceCategory),
        );
