...
function isDatabaseUnavailableError(err: unknown): boolean {
  ...
  // Connection / auth / SSL / timeout class errors from pg / Neon / PGLite
  return (
...
export const getAvailableSlots = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data }) => {
    ...
    // Do NOT catch DB errors here — propagate so the client can show "database unavailable"
    // instead of treating a failed query as "all slots free".
...
export const createBooking = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    try {
      ...
      const { getSql } = await import("@/lib/db");
      const sql = await getSql();
      // Schema changes belong in migrations/0004_bookings_user_id.sql — never ALTER at request time.
...
export const listMyBookings = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    try {
      ...
      const { getSessionUser } = await import("@/lib/auth/verify.server");
      const sql = await getSql();
      // user_id column is owned by migrations/0004_bookings_user_id.sql
