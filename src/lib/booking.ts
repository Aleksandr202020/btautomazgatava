...
/** Client cabinet: by user_id OR matching customer_email. */
export const listMyBookings = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    try {
      ...
      let rows: BookingRow[] = [];
      if (email) {
        rows = await sql<BookingRow>`
          select * from bookings
          where user_id = ${userId}
             or lower(coalesce(customer_email, '')) = ${email}
...
/** Cancel by user_id OR matching customer_email. */
...
/** Edit by user_id OR matching customer_email. */
