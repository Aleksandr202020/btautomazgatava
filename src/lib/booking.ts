...
function isDatabaseUnavailableError(err: unknown): boolean {
  ...
  return (
    /database.?unavailable|connection refused|econnrefused|etimedout|enotfound|connection terminated|connection timeout|ssl|password authentication failed|too many clients|could not connect|neon|pglite|database_url is not set|getaddrinfo|socket hang up|network|timeout exceeded/i.test(msg)
  );
}
...
export const getAvailableSlots = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data }) => {
    if (!isWorkingDate(data.date)) {
      return { ok: true as const, date: data.date, slots: [] as { time: string; free: boolean }[] };
    }
...
export const createBooking = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    // Always prefer account email from user table; form email is fallback contact only.
    let accountEmail: string | null = null;
    try {
      const { getSql } = await import("@/lib/db");
      const sql = await getSql();
      const urows = await sql<{ email: string }>`select email from "user" where id = ${context.userId} limit 1`;
      if (urows[0]?.email) accountEmail = String(urows[0].email).trim();
    } catch { accountEmail = null; }
    if (!accountEmail) {
      try {
        const { getSessionUser } = await import("@/lib/auth/verify.server");
        const su = await getSessionUser();
        accountEmail = su?.email?.trim() || null;
      } catch { accountEmail = null; }
    }
    ...
    try {
      ...
      const { getSql } = await import("@/lib/db");
      const sql = await getSql();
      // user_id is mandatory link to the account (Better Auth user id).
      ...
      if (err instanceof DatabaseUnavailableError || isDatabaseUnavailableError(err)) {
        console.error("[DB] createBooking: database unavailable");
...
export const adminCreateBooking = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    try {
      ...
      const { getSql } = await import("@/lib/db");
      const sql = await getSql();
      // Admin walk-in: no user_id (not linked to a client account).
      const inserted = await sql<BookingRow>`
        insert into bookings (
      ...
      if (isUniqueSlotError(err)) return { ok: false as const, error: "slot_taken" as const };
      if (err instanceof DatabaseUnavailableError || isDatabaseUnavailableError(err)) {
        console.error("[DB] adminCreateBooking: database unavailable");
        return { ok: false as const, error: "db_unavailable" as const };
      }
...
/** Client cabinet: list only by user_id. Email used once to claim orphan rows. */
export const listMyBookings = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    try {
      ...
      // One-time: attach orphan bookings (no user_id) that match account email.
      let email: string | null = null;
      try {
        const urows = await sql<{ email: string }>`select email from "user" where id = ${userId} limit 1`;
        if (urows[0]?.email) email = String(urows[0].email).trim().toLowerCase();
      } catch { /* ignore */ }
      if (!email) {
        try {
      ...
      console.error("[DB] listMyBookings failed", err);
      return { ok: false as const, bookings: [] as BookingPublic[], error: "db_unavailable" as const };
    }
  });

/** Cancel only own booking (user_id). */
    ...
    try {
      ...
      const updated = await sql<BookingRow>`
        update bookings set booking_date = ${nextDate}::date, booking_time = ${nextTime},
          ...
          user_id = ${context.userId}, updated_at = now()
      ...
      if (isUniqueSlotError(err)) return { ok: false as const, error: "slot_taken" as const };
      if (err instanceof DatabaseUnavailableError || isDatabaseUnavailableError(err)) {
        console.error("[DB] updateMyBooking: database unavailable");
        return { ok: false as const, error: "db_unavailable" as const };
      }
